...
void processA(int sock) {
	...
	*/
	...
	struct message mesg_a = {0, "A:Hello World"};
	...
	printf("\nEvent 1 of Process A: send msg to process B\n");
	...
	printf("Logical clock value of Process A after event 1: %d\n", mesg_a.logical_clock);


	printf("\nEvent 2 of Process A: recieve msg from process B\n");	
	/* Waiting for a message to be received */
	printf("Waiting to receive message...\n");
	err_handler(recv_message(sock, &recvd_from_b, sizeof(struct message), &mesg_a),
		"ReceivingError");

	printf("Logical clock value of Process A after event 2: %d\n", mesg_a.logical_clock);
...
void processB(int sock) {
	...
	*/
	...
	struct message mesg_b = {0, "B:Hello World"}; /*initialising the message*/
	/*Initial vlaue of logical_clock for each process is zero.*/
	...
	for(int i = 0; i < 2; i++) {
		printf("\nEvent %d of Process B: receive message\n", i + 1);
		printf("Waiting to receive message...\n");
	...
	printf("\nEvent 3 of Process B: send msg to process A\n");
	...
	printf("\nEvent 4 of Process B: send msg to process C\n");
	err_handler(send_message(sock, &mesg_b, sizeof(mesg_b), 0, 
		(struct sockaddr_in *) &recvr_address, sizeof(recvr_address)), "SendingError");
...
void processC(int sock) {
	...
	*/
	...
	printf("\nEvent 1 of Process C: send msg to process B\n");
	...
	printf("\nEvent 2 of Process C: receive msg from process B\n");
